...
def save_myTickers_tsv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    account_dir = ACCOUNT_ACTIVITY_DIR
    # Filter files from last 3 months
    three_months_ago = datetime.now() - timedelta(days=90)
    
    # Get all position files and filter by date
    files = []
    for file_pattern in ['Portfolio_Positions*.csv', 'Fidelity_Positions_*.csv']:
        for file_path in account_dir.glob(file_pattern):
            # Try to extract date from filename
            try:
                filename = file_path.name
                if 'Portfolio_Positions_' in filename:
                    date_part = filename.split('_')[2].replace('.csv', '')
                    file_date = datetime.strptime(date_part, '%Y%b%d')
                elif 'Fidelity_Positions_' in filename:
                    date_part = filename.split('_')[2].replace('.csv', '')
    ...
    if files:
        newest_file = files[0]  # Already sorted by date (newest first)
        ...
        if df.empty or 'Symbol' not in df.columns:
            ...
            if 'Quantity' in raw_df.columns:
                ...
                print(f"Filtered out closed positions, {len(raw_df)} positions remain")
